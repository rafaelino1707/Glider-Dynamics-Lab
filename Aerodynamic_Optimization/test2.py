...
for mass in masses:
    for chord in chord_vals:
        for span in span_vals:
            ...
            is_clamped = int(g.cl_min_sink > g.cl_max)
            rows.append({
                "mass_kg": mass,
                "chord_m": chord,
                "span_m": span,
...
g = Glider(0.2, 0.6, 0.02, 0.06, cl_max=1.7187, oswald_efficiency=OSWALD_EFF, cd0=CD0)
print("Vms =", g.v_min_sink())
print("sink_min =", g.sink_min()["sink_min"])
print("Vstall =", g.v_stall())
# ---------------------------- Plotting ------------------------------------
