...
# INPUT / OUTPUT
# -----------------------------
CSV_PATH = "log/20251214_201205_glider_ble.csv"
OUT_DIR = "log2"
OUT_SUFFIX = "_integrated_nozupt"
...
LPF_CUTOFF_HZ = 7.0                 # kill high-f noise
HAMPEL_WIN_S  = 0.15                # kills 1–2 sample glitches
HAMPEL_NSIGMA = 4.0

# Drift-killers WITHOUT ZUPT:
HPF_CUTOFF_HZ = 0.10                # remove slow accel bias/drift (~>10 s timescale)
VEL_LEAK_TAU_S = 6.0                # leaky integrator time constant (bigger = less damping)
...
def main():
    ...
    # High-pass (remove slow drift/bias without any ZUPT logic)
    a_hpf = hpf_1pole(a_clean, t, HPF_CUTOFF_HZ)

    # Jerk flags (on the final accel used for integration)
    jerk_mag, jerk_isolated = compute_jerk_flags(a_hpf, dt_full, JERK_MAX, JERK_WIN)

    # -----------------------------
    # INTEGRATION (NO ZUPT)
    # - leaky integrator to limit velocity random walk
    ...
    for i in range(1, N):
        ...
        # leaky integrator on velocity: v <- v*exp(-dt/tau) + a*dt
        if VEL_LEAK_TAU_S is not None and VEL_LEAK_TAU_S > 1e-6:
            leak = np.exp(-dt_i / VEL_LEAK_TAU_S)
        else:
            leak = 1.0
        ...
        # trapezoidal on position using v_prev, v_i
    ...
    print(f"LPF={LPF_CUTOFF_HZ}Hz | Hampel win={HAMPEL_WIN_S}s nsigma={HAMPEL_NSIGMA} | HPF={HPF_CUTOFF_HZ}Hz")
    print(f"Vel leak tau={VEL_LEAK_TAU_S}s | Jerk gate: JERK_MAX={JERK_MAX} win={JERK_WIN}")
    ...
    with open(out_path, "w", newline="") as f:
        ...
        f.write(f"# note=NO_ZUPT; drift control via HPF on accel + leaky velocity integrator\n")
